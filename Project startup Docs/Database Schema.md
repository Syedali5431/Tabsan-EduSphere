...
## 2026-06-04 Update - ISO Phase 5 Data Protection (Schema Posture)
- Implementation: new table `data_classification_entries` (EntityName, EntityId, ClassificationLevel, ClassifiedBy, ClassifiedAt, Justification + BaseEntity). Indexes: IX_data_classification_entity, IX_data_classification_level_classified. Added ConsentToMonitoring (bit null) + DataRetentionDate (datetime2 null) to users.
- Validation: EF migration `*_PhaseISO5DataProtection`, additive-only.
...
- Implementation Summary:
	- completed web-layer Phase 4 continuation updates for Attendance period handling and write-gating behavior,
...
- Implementation Summary:
	- advanced Scripts/03-FullDummyData.sql marker to `FullDummyData-v43`,
	- added additive deterministic payment student-scope demo rows in existing `payment_receipts` table only:
		- `RCPT-DEMO-PAY-SCP-U-043`,
		- `RCPT-DEMO-PAY-SCP-C-043`,
		- `RCPT-DEMO-PAY-SCP-S-043`,
	- synchronized Scripts/05-PostDeployment-Checks.sql with v43 marker and deterministic institution-alignment checks for the new scope-demo rows.
- Validation Summary:
...
- Implementation Summary:
	- Updated Scripts/03-FullDummyData.sql dataset marker to FullDummyData-v32.
	- Expanded additive deterministic Generate Certificates demo rows in existing tables only:
		- users,
		- student_profiles,
		- enrollments.
	- Updated Scripts/05-PostDeployment-Checks.sql with additive v32 marker and expanded deterministic cohort assertions.
- Validation Summary:
	- SQL updates are additive and idempotent at row level for deterministic IDs.
	- Schema impact: No schema mutation.
- EF migration impact: none.
...
- Implementation Summary:
	- Updated Scripts/03-FullDummyData.sql with additive deterministic rows in existing quizzes table for offering 55555555-5555-5555-5555-555555555501:
		- 13131313-1313-1313-1313-131313131307 (active),
		- 13131313-1313-1313-1313-131313131308 (inactive).
	- Updated Scripts/05-PostDeployment-Checks.sql with additive quiz demo assertions:
	...
	- Runtime synchronization for quiz identity/action/filter behavior used existing contracts and existing tables.
- Validation Summary:
	- SQL updates are additive and idempotent.
	- Quizzes filter checks now have deterministic seeded active/inactive rows for demo/testing.
- Schema impact: No schema mutation.
- EF migration impact: none.

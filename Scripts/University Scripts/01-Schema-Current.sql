...
/*
  University schema bootstrap script (pure T-SQL).
...
BEGIN
  PRINT N'University schema note: base schema tables are missing.';
  PRINT N'Run shared schema script once: Scripts/01-Schema-Current.sql (prefer SQLCMD mode), then rerun this University pack.';
  RETURN;
END;
...
BEGIN
  IF COL_LENGTH('discussion_threads', 'IsSolved') IS NULL
    ALTER TABLE [discussion_threads] ADD [IsSolved] BIT NOT NULL CONSTRAINT [DF_university_discussion_threads_IsSolved] DEFAULT ((0));

  IF COL_LENGTH('discussion_threads', 'IsVisibleToAll') IS NULL
    ALTER TABLE [discussion_threads] ADD [IsVisibleToAll] BIT NOT NULL CONSTRAINT [DF_university_discussion_threads_IsVisibleToAll] DEFAULT ((0));
  ...
  IF COL_LENGTH('discussion_threads', 'ThreadType') IS NULL
    ALTER TABLE [discussion_threads] ADD [ThreadType] NVARCHAR(30) NOT NULL CONSTRAINT [DF_university_discussion_threads_ThreadType] DEFAULT (N'Issue');

  IF COL_LENGTH('discussion_threads', 'TicketNumber') IS NULL
    ALTER TABLE [discussion_threads] ADD [TicketNumber] NVARCHAR(40) NOT NULL CONSTRAINT [DF_university_discussion_threads_TicketNumber] DEFAULT (N'');
END;

PRINT N'University schema wrapper validation passed.';
GO
